...
class HMM:
    ...
    def viterbi(self, Osequence):
        ...
        L = len(Osequence)
        delta = np.zeros((S, L))
        max_idx = np.zeros_like(delta, dtype=int)
        delta[:, 0] = self.pi * self.B[:, self.obs_dict[Osequence[0]]]

        for t in range(1, L):
            temp = self.A * np.array(delta[:, t - 1])[:, None]
            max_idx[:, t] = np.argmax(temp, axis=0)
            delta[:, t] = self.B[:, self.obs_dict[Osequence[t]]] * np.max(temp, axis=0)
        path_idx.append(np.argmax(delta[:, -1]))
